    ...
    <div className="space-y-8">
      ...
      <div className="flex border-b border-gold/10 gap-4 overflow-x-auto no-scrollbar">
        ...
        <button 
          ...
          className={`pb-3 text-xs tracking-wider uppercase border-b-2 font-medium ${subTab === 'ai-advisor' ? 'text-gold border-gold' : 'text-muted border-transparent hover:text-cream'}`}
        >
          AI Phan Tich Doanh Nghiep
        </button>
        <button 
        ...
        <button 
          ...
          className={`pb-3 text-xs tracking-wider uppercase border-b-2 font-medium ${subTab === 'users' ? 'text-gold border-gold' : 'text-muted border-transparent hover:text-cream'}`}
        >
          Quan Ly Nhan Vien
        </button>
        <button 
        ...
        <div className="space-y-8">
          ...
          {summary && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="border border-gold/10 p-5 bg-bg-2">
      ...
      {/* AI BUSINESS ADVISOR */}
      {subTab === 'ai-advisor' && (
        <div className="border border-gold/10 bg-bg-2 p-6 flex flex-col h-[520px] justify-between">
          <div className="flex items-center gap-3 pb-4 border-b border-gold/10">
            <Sparkles size={16} className="text-gold" />
            <h3 className="font-serif text-base text-gold uppercase tracking-wider">
              Sakura Business Advisor — Phan tich & Khuyen nghi doanh nghiep
            </h3>
          </div>
          ...
          <div className="flex-1 overflow-y-auto my-4 pr-2 space-y-4 no-scrollbar">
            {aiChat.map((msg, index) => (
              <div 
                key={index}
                className={`max-w-[85%] p-4 text-xs leading-relaxed whitespace-pre-line border rounded-none ${
                  msg.role === 'user' 
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-gold/10">
            <input 
              type="text" 
              placeholder="Hoi ve doanh thu thang nay, mon an ban chay nhat, hoac khung gio vang..."
              className="flex-1 bg-transparent border-none outline-none text-xs text-cream placeholder-muted"
              value={aiInput}
              onChange={(e) => setAiInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSendAIConsultant()}
              disabled={aiLoading}
            />
            <button 
              onClick={handleSendAIConsultant}
              disabled={aiLoading}
